...
void busca2(int i, TNo *Times){
    float custo=0;
    printf(" Busca de aprofundamento iterativo\n\n         Time %s Posicao %d\n\n\n", Times[i].time,Times[i].classificado);
    int mnota=0;
    int pOp;
    ...
    for(int ii=0;ii<3;ii++){
        ...
        printf("Adversario das quartas: %s (%.1f) [ELIMINADO]\n", Times[quartas[ii]].time,Times[quartas[ii]].nota);
        custo=Times[quartas[ii]].nota;
        printf("Dificuldade do caminho: %.1f\n", custo);
            ...
            for(int jj=0;jj<6;jj++){
                for(int j=0;j<6;j++){
                    if(Times[j].classificado!=0 && Times[j].time!= Times[i].time){}
                       // printf("Possivel adversario na semi:  %s (%.1f)\n", Times[Semi[j]].time, Times[Semi[j]].nota );
                    }
                printf("Adversario da semi: %s (%.1f) [ELIMINADO] \n", Times[Semi[jj]].time, Times[Semi[jj]].nota);
                custo2=custo+Times[Semi[jj]].nota;
                printf("Custo desse caminho (quartas %s x %s e semi %s x %s): %.1f\n", Times[i].time, Times[quartas[ii]].time, Times[i].time,Times[Semi[jj]].time,custo2);
                ...
                for(int j=0;j<16;j++){
                    if(Times[j].time!=Times[i].time && Times[j].classificado != 0){
                        printf("Possivel adversario na final: %s (custo total: %.1f)\n", Times[j].time, Times[j].nota+custo2 );
...
int main(){
   ...
   for(i=0;i<8;i++){
    ...
    for(i=0;i<16;i++){
        Times[i].nota=Times[i].nota+Times[i].vitorias;
    }
    for(i=0;i<8;i++){
        busca2(c[i],Times);
    }
